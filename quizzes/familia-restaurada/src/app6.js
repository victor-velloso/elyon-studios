...
var ICONS = {
  ...
  pix:"<path fill='currentColor' stroke='none' d='M12 3.2 14.6 5.8 12 8.4 9.4 5.8z'/><path fill='currentColor' stroke='none' d='M18.2 9.4 20.8 12 18.2 14.6 15.6 12z'/><path fill='currentColor' stroke='none' d='M12 15.6 14.6 18.2 12 20.8 9.4 18.2z'/><path fill='currentColor' stroke='none' d='M5.8 9.4 8.4 12 5.8 14.6 3.2 12z'/>"
};
function ico(name, px){
  var p = ICONS[name] || ICONS.check;
  var s = px || 22;
  return "<svg class='fr-ico' viewBox='0 0 24 24' width='"+s+"' height='"+s+"' style='width:"+s+"px !important;height:"+s+"px !important;min-width:"+s+"px;min-height:"+s+"px;max-width:none;flex:none;display:block;overflow:visible' fill='none' stroke='currentColor' stroke-width='1.75' stroke-linecap='round' stroke-linejoin='round' aria-hidden='true'>"+p+"</svg>";
}
...
function areaMark(area, px){
  var name = {C:"heart", F:"sprout", O:"hands", D:"sun"}[area] || "check";
  var s = px || 28;
  var inner = Math.round(s * 0.62);
  return "<span class='fr-icirc' style='width:"+s+"px;height:"+s+"px;min-width:"+s+"px;flex:0 0 "+s+"px'>"+ico(name, inner)+"</span>";
}
...
function renderResult(el){
  ...
  var front = "";
  if(r.second){
    front = "<section class='fr-blk fr-dx-sec'><p class='fr-front'>"+areaMark(r.second, 28)+"<span><b>Também pesa aí: "+AREA2_TXT(r.second)+".</b> Quando tudo aperta junto, não é coincidência.</span></p></section>";
  }
...
function renderPitch(el){
  ...
  var relatos = [
    {k:"relato1", who:"@katianascimento9902", crop:"r1"},
    {k:"relato2", who:"@andrezarosolen", crop:"r2"},
    {k:"relato3", who:"@maria.rgoncalves", crop:"r3"}
  ].map(function(x){
    var u = cfgImg(x.k);
    var im = u ? "<span class='fr-rcrop "+x.crop+"'><img class='fr-fall' data-drop='img' loading='lazy' alt='Relato de "+esc(x.who)+"' src='"+esc(u)+"'></span>" : "";
  ...
  var faqQuem;
  if(r.main==="O" || r.main==="D") faqQuem = "<details class='fr-acc'><summary>E se ninguém em casa mudar?</summary><p>A sua oração não depende de ninguém mudar primeiro. Você faz a sua parte com direção. O resto é com Deus.</p></details>";
  else faqQuem = "<details class='fr-acc'><summary>E se "+ele()+" não quer mudar?</summary><p>Você responde pela sua oração, não pelas escolhas "+dele()+". Interceder é orar por quem não ora.</p></details>";
  faq += faqQuem
  ...
  var also = r.second ? "<p class='fr-sum-second'>"+areaMark(r.second, 28)+" também: "+AREA2_TXT(r.second)+"</p>" : "";
  var p3fecha = LUTOU() ? "Não foi falta de fé. Foi o mesmo remédio pra tudo." : "Não é falta de fé. Faltou o remédio certo.";
  var passo3txt;
  if(r.main==="O" || r.main==="D") passo3txt = "Em voz alta, com o seu nome e o de quem você está cobrindo, e um passo hoje.";
  else passo3txt = "Em voz alta, com o nome "+(r.main==="C" ? dele() : "dele")+", e um passo hoje.";
  el.innerHTML = "<div class='fr-pitch'>"
    ...
    + "<li><span class='fr-num'>3</span><div><b>Falar a Palavra e fazer a sua parte.</b><p>"+esc(passo3txt)+"</p></div></li>"
    + "</ol>"
    ...
    + "<p class='fr-pag'>"+ico("card", 18)+" Pagamento único.</p>"
    + "<ul class='fr-sealrow'>"
  ...
  function fitCar(){
    if(!trackEl) return;
    var slides = trackEl.children, w = trackEl.clientWidth || 1, i = Math.round(trackEl.scrollLeft / w), s;
    if(i < 0) i = 0;
    if(ge(i, slides.length)) i = slides.length - 1;
    s = slides[i];
    if(s) trackEl.style.height = s.offsetHeight + "px";
  }
  ...
  if(trackEl){
    trackEl.addEventListener("scroll", fitCar, {passive:true});
    window.addEventListener("resize", fitCar);
    trackEl.querySelectorAll("img").forEach(function(im){ im.addEventListener("load", fitCar); });
    fitCar();
  }
  bindMedia(el);
  fadeIn(el);

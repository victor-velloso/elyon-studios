...
const P = JSON.parse(fs.readFileSync(__dirname + '/paths-shots.json', 'utf8'));
...
(async () => {
  ...
  for (const path of P) {
    ...
    const VP=process.env.VP||'mobile'; if(VP==='desktop') await page.setViewport({ width: 1440, height: 900, deviceScaleFactor: 1 }); else await page.setViewport({ width: 390, height: 844, deviceScaleFactor: 2, isMobile: true, hasTouch: true });
    ...
    for (let guard = 0; guard < 80; guard++) {
      ...
      if (SHOTS && path.shots && path.shots.includes(step)) {
        await new Promise(r=>setTimeout(r, step==='t35-resultado'?1200:500));
        await page.screenshot({ path: `${SHOTS}/v-nova-${process.env.VP||'mobile'}-${path.id.split('-')[0]}-${step}.png` });
        if (step === 't35-resultado' || step === 't41-pitch') { await page.evaluate(()=>document.querySelectorAll('.fr-dx-sec').forEach(e=>e.classList.add('in'))); await new Promise(r=>setTimeout(r,1200)); await page.screenshot({ path: `${SHOTS}/v-nova-${process.env.VP||'mobile'}-${path.id.split('-')[0]}-${step}-full.png`, fullPage: true }); }
      }
      ...
      else if (step === 't35-resultado') { await page.evaluate(async () => { for (let y = 0; y < document.body.scrollHeight; y += 300) { window.scrollTo(0, y); await new Promise(r => setTimeout(r, 60)); } }); await new Promise(r => setTimeout(r, 300)); if (SHOTS && path.shots && path.shots.includes('t35-resultado')) { await page.screenshot({ path: `${SHOTS}/v-nova-${process.env.VP||'mobile'}-${path.id.split('-')[0]}-t35-resultado-full.png`, fullPage: true }); } await page.click('#fr-next'); }
    ...
    if (SHOTS && path.shots) {
      if (path.shots.includes('t41-pitch-price')) { await page.evaluate(() => document.querySelector('.fr-price').scrollIntoView({block:'center'})); await new Promise(r=>setTimeout(r,500)); await page.screenshot({ path: `${SHOTS}/v-nova-${process.env.VP||'mobile'}-${path.id.split('-')[0]}-t41-pitch-price.png` }); }
    }
  ...
  fs.writeFileSync(__dirname + '/results-shots.json', JSON.stringify(results, null, 1));
  await browser.close();
})();
